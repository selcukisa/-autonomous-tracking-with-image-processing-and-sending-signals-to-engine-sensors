...
# Motor kontrol thread'i
def motor_control_thread():
    while True:
        target_pos = target_queue.get()
        if target_pos is None:
            break
        with current_pos_lock:
            motor.move_to_target(current_pos, target_pos)
...
detection_thread = threading.Thread(target=detection_and_tracking_thread, daemon=True)
